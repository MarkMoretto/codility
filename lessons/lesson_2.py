...
Challenge:
    ...
    moved to the first place. For example, the rotation of array A = [3, 8, 9, 7, 6] is
...
def solution(A, K):
    if isinstance(A, list):
        ...
        print("Please pass a list as argument A.")
...
class TestRotateArrays(unittest.TestCase):
    ...
    def test_rotate_by_three(self):
        a = [3, 8, 9, 7, 6]
        k = 3
        self.assertEqual(solution(a, k), [9, 7, 6, 3, 8])

    def test_rotate_all_zeroes_by_one(self):
        a = [0, 0, 0]
        k = 1
        self.assertEqual(solution(a, k), [0, 0, 0])

    def test_rotate_to_initial_order(self):
        a = [1, 2, 3, 4]
        k = 4
